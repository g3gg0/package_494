...
static void print_switch_values (print_switch_fn_type);
static int print_single_switch (print_switch_fn_type, int, print_switch_type,
				const char*);
  ...
  if (flag_generate_lto)
    {
      ...
      {
	/* Output version information to .version_info section as requested by
	   -mversion-info resp. -mversion-string=<info>.  Some types are
	   provided by target.h but we don't want to include target.h in
	   htc-common.h.  Thus, supply the prototype by hand.  */

	extern void htc_asm_out_version_info
	  (void (*)(print_switch_fn_type),
	   int (*)(print_switch_fn_type, int, print_switch_type, const char*),
	   const char*);

	htc_asm_out_version_info (print_switch_values, print_single_switch,
				  TARGET_NAME);
      }
...
process_options (void)
{
  ...
  if (flag_gtoggle)
    {
      if (flag_dwarf_control_flow)
        error("Conflicting options. Can not use %qs with %qs",
          "-fdwarf-control-flow", "-gtoggle");
...
#ifdef WITH_HIGHTEC
  htc_set_error_numbers (argc, argv);
#endif /* WITH_HIGHTEC */
